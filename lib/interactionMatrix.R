...
interaction.matrix <- function(trait, intercept=1, mu=0, sigma=0.1, intra=0, std = F){
  # Arguments
...
interaction.matrix_bi <- function(trait1, trait2, intercept=1, mu1=0, sigma1=0.1, mu2 = 0, sigma2 = 0.1, rho = 0, intra= 1, std = F){
  # Arguments
  ...
  if (std){
    A <- iintercept * exp(A)/(2*pi*sigma1*sigma2*sqrt(1-rho^2))
  }else{
    A <- intercept * exp(A)
  }
  diag(A) <- intra
